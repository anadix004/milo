...
export default function NotificationSidebar({ isOpen, onClose }: NotificationSidebarProps) {
    ...
    <div className={clsx("flex flex-col h-full", !isMobile && "p-0")}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-4">
        {notifications.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center opacity-20 space-y-4 text-center px-12 py-20">
  ...
  if (isMobile) {
    return (
      <BottomSheet isOpen={isOpen} onClose={onClose} snapHeight="90vh">
        {renderContent()}
      </BottomSheet>
    );
  }
